...
//SEIN WAEVS
	...
	//IM LIVID FUCKING SINE WAVES CODE
	...
	float distanceSqr = xDis*xDis + yDis*yDis; // circle mask
	                  //vec3    //transparency
	if(distanceSqr <= (Factor4*0.1)*radius + 10000.0)
	{
		deThiccV = 0.1; // how THICC it is vertically
		deThiccH = 1.0; // how THICC it is horizontally
		deSpeedMultiplier = 3.0; // how FAST the sine wave travels
		deHeight = 0.5; // how HIGH the sine wave is
		deFattMan = 1.0; // how FATT the sine wave is
		deIDKWhat = 1.0; // Idk what this effect is called
	
		color1 = vec3(1.0, 0.2, 0.3); // line color
		color2 = vec3(0.4, 0.5, 1.0); // gradient color
		resolution = vec2(800.0, 600.0); // the resoltuion
		xCenter = (gl_FragCoord.y * gl_FragCoord.y + gl_FragCoord.x) * Factor4 / resolution.y; // sin wave X pixel
		yCenter = gl_FragCoord.x / resolution.x; // sin wave Y pixel
		//xCenter = gl_FragCoord.y * Factor4 / resolution.y; // sin wave X pixel
		curve = deThiccV * sin((deThiccH * xCenter * Factor4) + (deSpeedMultiplier * Factor1));
		line = smoothstep(1.0 - clamp(distance(curve+yCenter, 0.5) * deFattMan, 0.0, 1.0), deIDKWhat, 0.99);
		sineWave = (1.0 - line) * vec3(mix(color1 * (Factor4/0.5), color2 * (-Factor4*0.1), line));
		gl_FragColor = vec4(sineWave, 1.0) * texColor;
	}
	else
	{
		deThiccV = 0.1; // how THICC it is vertically
		deThiccH = 10.0; // how THICC it is horizontally
		deSpeedMultiplier = 2.0; // how FAST the sine wave travels
		deHeight = 0.5 * (Factor4 * 0.2); // how HIGH the sine wave is
		deFattMan = 1.0; // how FATT the sine wave is
		deIDKWhat = 1.0; // Idk what this effect is called
	
		color1 = vec3(0.4, 0.5, 0.2); // line color
		color2 = vec3(0.1, 0.3, 0.2); // gradient color
		resolution = vec2(800.0, 600.0); // the resoltuion
		xCenter = gl_FragCoord.x / resolution.x; // sin wave X pixel
		yCenter = gl_FragCoord.y / resolution.y; // sin wave Y pixel
		curve = deThiccV * sin((deThiccH * xCenter * Factor4) + (deSpeedMultiplier * Factor1));
		line = smoothstep(1.0 - clamp(distance(curve+yCenter * Factor4, 0.5) * deFattMan, 0.0, 1.0), deIDKWhat, 0.99);
		sineWave = (1.0 - line) * vec3(mix(color1, color2 * (Factor4 / 0.5), line));
		gl_FragColor = vec4(sineWave, 1.0) / texColor;
	}
